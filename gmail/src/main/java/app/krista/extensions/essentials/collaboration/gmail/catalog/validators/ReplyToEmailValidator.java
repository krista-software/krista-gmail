...
/**
 * Validator for "Reply-To" email addresses in Gmail operations.
 * Validates email address format and collects invalid addresses for error reporting.
 */
public class ReplyToEmailValidator implements Validator {

    private static final List<EmailAddress> emailAddresses = new ArrayList<>();

    /**
     * Validates that all Reply-To email addresses have valid format.
    ...
    public String getFetchFieldName() {
        return GmailResources.REPLY_TO;
    }
    ...
    public String getFetchStepMessage() {
        return "Please enter a valid reply-to email address in the format: user@domain.com";
    }

    @Override
    public String getConfirmationStepMessage(String resourceId, Map<ValidationResource, String> context) {
        return String.format("The reply-to email address '%s' could not be validated. Please verify the address and try again.", toStringMailIds());
    }

    @Override
    public String getErrMessage(String resourceId) {
        ValidationResourceUtil.toEmailAddresses(resourceId, emailAddresses);
        return String.format("The 'Reply-To' email addresses are not valid: %s. Please check the format and try again.", toStringMailIds());
    }
    ...
    private static String toStringMailIds() {
        String mailIds = ReplyToEmailValidator.emailAddresses.stream()
                .map(EmailAddress::getEmailAddress)
                .collect(Collectors.joining(", "));
        ReplyToEmailValidator.emailAddresses.clear();
        return mailIds;
    }
}